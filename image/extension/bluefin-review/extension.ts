...
export function actionPrompt(action: DashboardAction, priority?: Priority): string | undefined {
	...
	if (batch) {
		...
		// Slay/landing rules are write authority: commit, PR delivery, merge, and
		// conflict repair. They travel only inside the verbatim block a write
		// action fans out to its subagents, never into a read-only prompt. Split
		// by action authority at this seam (#479.1).
		const isReadOnly = action.kind === "review" || action.kind === "diff";
		const writeBrief = `${evidenceRule} ${noPollRule} ${conflictRule} Skip formatters, linters, and project-wide suites; run only the smallest existing test covering what changed. ${SLAY_DELIVERY_RULE} The bracketed queue read travels with your item: it is triage evidence, and you revalidate head and checks live before any approve, merge, label, or push.`;
		const readBrief = `${evidenceRule} ${noPollRule} Skip formatters, linters, and project-wide suites; run only the smallest existing test covering what changed. The bracketed queue read travels with your item: it is triage evidence. Report what you verified and what you could not, and do not implement, commit, merge, or deliver anything.`;
		const fanOut = `Work all ${batch.length} items with ONE subagent per issue/PR, capped at a maximum of 7 concurrent subagents at any time (queue remaining items and dispatch as running subagents complete; review/landing agents do not count against the 7 cap). Each subagent owns exactly its assigned item and its bracketed queue read. A subagent that stops instead of waiting frees its concurrency slot. Every subagent prompt MUST end with the block between the markers below, copied verbatim — do not summarise or omit it:\n<<<SUBAGENT-RULES\n${isReadOnly ? readBrief : writeBrief}\nSUBAGENT-RULES>>>\nReport per item — what you did, the evidence, and the outcome.`;

		// Shared neutral mechanics (the 7-subagent fan-out) ride both. Write
		// authority — snapshot revalidation, conflict repair, native-mutation
		// preference — is only meaningful where an action can mutate, so it stays
		// out of the read-only protocol (#479.1).
		const writeProtocol = `${snapshotRule}\n\n${conflictRule}\n\n${fanOut}\n\n${mutationRule}\n\n${autonomousRule}`;
		const readProtocol = `${fanOut}\n\n${autonomousRule}`;
		switch (action.kind) {
			case "review":
